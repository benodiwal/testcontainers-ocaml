(** Docker API client abstraction layer.
    ...
    This module provides an abstraction over Docker API communication.
    Uses direct Unix socket connection via Lwt_unix.
    Designed to be swappable for future Eio implementation. *)
...
let read_http_response ic =
  ...
  let content_length =
    List.find_map (fun h ->
      ...
      if String.length h_lower > 15 &&
         String.sub h_lower 0 14 = "content-length" then
        let colon_pos = String.index h ':' in
        let value = String.trim (String.sub h (colon_pos + 1) (String.length h - colon_pos - 1)) in
...
let make_request meth path ?body () =
  ...
  let content_length = match body with
    | Some b -> String.length b
    | None -> 0
  in
  let request =
    Printf.sprintf "%s %s HTTP/1.1\r\nHost: localhost\r\nContent-Type: application/json\r\nContent-Length: %d\r\nConnection: close\r\n\r\n%s"
      meth_str full_path content_length (Option.value body ~default:"")
  in
...
let config_to_json config =
  let exposed_ports =
    List.map (fun p -> (p, `Assoc [])) config.exposed_ports
    |> fun lst -> `Assoc lst
  in
  let port_bindings =
    List.map (fun (port, bindings) ->
      let bindings_json = List.map (fun b ->
        `Assoc [
  ...
  let base = match config.cmd with
    | Some cmd -> ("Cmd", `List (List.map (fun s -> `String s) cmd)) :: base
    | None -> base
  in
  let base = match config.entrypoint with
    | Some ep -> ("Entrypoint", `List (List.map (fun s -> `String s) ep)) :: base
    | None -> base
  in
  let base = match config.working_dir with
    | Some wd -> ("WorkingDir", `String wd) :: base
    | None -> base
  in
  let base = match config.user with
...
let parse_create_response body_str =
  ...
  let warnings =
    ...
    | Some lst -> List.filter_map (function `String s -> Some s | _ -> None) lst
    | None -> []
  in
  { id; warnings }
...
let parse_port_bindings json =
  match json with
  | `Assoc lst ->
      List.filter_map (fun (port, bindings) ->
        match bindings with
        | `List bl ->
            let parsed_bindings = List.filter_map (fun b ->
              match b with
              | `Assoc _ ->
                  let host_ip = Json.get_string "HostIp" b |> Option.value ~default:"" in
                  let host_port = Json.get_string "HostPort" b |> Option.value ~default:"" in
...
let parse_container_info body_str =
  ...
  let id = Json.get_string_exn "Id" json in
  let name =
    Json.get_string "Name" json
    |> Option.value ~default:""
    |> fun s -> if String.length s > 0 && s.[0] = '/' then String.sub s 1 (String.length s - 1) else s
  in
  let state_json = Json.get_assoc "State" json |> Option.value ~default:(`Assoc []) in
  let state = {
    status = Json.get_string "Status" state_json |> Option.value ~default:"unknown";
    running = Json.get_bool "Running" state_json |> Option.value ~default:false;
    exit_code = Json.get_int "ExitCode" state_json |> Option.value ~default:0;
  } in
  let network_json = Json.get_assoc "NetworkSettings" json |> Option.value ~default:(`Assoc []) in
  let ports_json = Json.get_assoc "Ports" network_json |> Option.value ~default:(`Assoc []) in
  let network_settings = {
    ports = parse_port_bindings ports_json;
    ip_address = Json.get_string "IPAddress" network_json |> Option.value ~default:"";
    gateway = Json.get_string "Gateway" network_json |> Option.value ~default:"";
  } in
...
let container_logs ?(stdout = true) ?(stderr = true) ?(follow = false) ?(tail = "all") id =
  let path = Printf.sprintf "/containers/%s/logs?stdout=%b&stderr=%b&follow=%b&tail=%s"
    id stdout stderr follow tail in
  let* resp = make_request `GET path () in
...
let wait_container id =
  ...
  let* body_str = handle_response resp in
  let json = Yojson.Safe.from_string body_str in
  let exit_code = Json.get_int "StatusCode" json |> Option.value ~default:(-1) in
...
let pull_image image =
  let path = Printf.sprintf "/images/create?fromImage=%s" (Uri.pct_encode image) in
  let* resp = make_request `POST path () in
  let* _ = handle_response resp in
  Lwt.return_unit
