...
let with_env key value t =
  { t with env = (key, value) :: t.env }

let with_envs envs t =
  { t with env = envs @ t.env }

let with_cmd cmd t =
  { t with cmd = Some cmd }

let with_entrypoint entrypoint t =
  { t with entrypoint = Some entrypoint }

let with_mount mount t =
  { t with mounts = mount :: t.mounts }

let with_mounts mounts t =
  { t with mounts = mounts @ t.mounts }

let with_network network t =
  { t with network = Some network }

let with_network_alias alias t =
  { t with network_aliases = alias :: t.network_aliases }

let with_name name t =
  { t with name = Some name }

let with_label key value t =
  { t with labels = (key, value) :: t.labels }

let with_labels labels t =
  { t with labels = labels @ t.labels }

let with_working_dir dir t =
  { t with working_dir = Some dir }

let with_user user t =
  { t with user = Some user }

let with_privileged privileged t =
  { t with privileged }

let with_wait_strategy strategy t =
  { t with wait_strategy = Some strategy }

let with_startup_timeout timeout t =
  { t with startup_timeout = timeout }

let with_auto_remove auto_remove t =
  { t with auto_remove }
