...
let test_bind_mount _switch () =
  ...
  Alcotest.(check string)
    "docker format" "/host/path:/container/path:rw"
    (Volume.to_docker_bind_format mount);
  Lwt.return_unit

let test_bind_mount_readonly _switch () =
  let mount =
    Volume.bind ~mode:Volume.ReadOnly ~host:"/host/path"
      ~container:"/container/path" ()
  in
  Alcotest.(check string)
    "docker format readonly" "/host/path:/container/path:ro"
    (Volume.to_docker_bind_format mount);
  Lwt.return_unit
