...
let test_for_listening_port _switch () =
  ...
  Alcotest.(check bool) "name contains port" true (String.length (Wait_strategy.name strategy) > 0);
  Alcotest.(check (float 0.1)) "default timeout" 60.0 (Wait_strategy.timeout strategy);
  Lwt.return_unit

let test_for_log _switch () =
  let strategy = Wait_strategy.for_log "ready to accept connections" in
  Alcotest.(check bool) "name set" true (String.length (Wait_strategy.name strategy) > 0);
  Lwt.return_unit

let test_for_log_with_occurrence _switch () =
  let strategy = Wait_strategy.for_log ~occurrence:2 "ready" in
  Alcotest.(check bool) "name contains occurrence" true (String.length (Wait_strategy.name strategy) > 0);
  Lwt.return_unit
...
let test_for_http_with_options _switch () =
  let strategy = Wait_strategy.for_http ~port:(Port.tcp 8080) ~status_codes:[200; 201] "/health" in
  Alcotest.(check bool) "name set" true (String.length (Wait_strategy.name strategy) > 0);
  Lwt.return_unit
...
let test_with_timeout _switch () =
  ...
  in
  Alcotest.(check (float 0.1)) "custom timeout" 30.0 (Wait_strategy.timeout strategy);
  Lwt.return_unit
...
let test_with_poll_interval _switch () =
  ...
  in
  Alcotest.(check bool) "strategy created" true (String.length (Wait_strategy.name strategy) > 0);
  Lwt.return_unit
...
let test_all_combinator _switch () =
  ...
  Alcotest.(check bool) "combined name contains 'all'" true (String.sub (Wait_strategy.name combined) 0 3 = "all");
  Lwt.return_unit
...
let test_any_combinator _switch () =
  ...
  Alcotest.(check bool) "combined name contains 'any'" true (String.sub (Wait_strategy.name combined) 0 3 = "any");
  Lwt.return_unit
...
let suite =
  [
    ...
    Alcotest_lwt.test_case "for_log with occurrence" `Quick test_for_log_with_occurrence;
    Alcotest_lwt.test_case "for_log_regex" `Quick test_for_log_regex;
    Alcotest_lwt.test_case "for_http" `Quick test_for_http;
    Alcotest_lwt.test_case "for_http with options" `Quick test_for_http_with_options;
