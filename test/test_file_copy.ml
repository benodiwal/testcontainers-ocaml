...
let test_copy_content_to_container _switch () =
  if Test_helpers.skip_integration_tests () then Lwt.return_unit
    ...
    Container.with_container request (fun container ->
      ...
      let* () = Container.copy_content_to container ~content ~dest:"/tmp/test.txt" in
      let* (exit_code, output) = Container.exec container ["cat"; "/tmp/test.txt"] in
...
let test_copy_file_to_container _switch () =
  if Test_helpers.skip_integration_tests () then Lwt.return_unit
    ...
    Container.with_container request (fun container ->
      ...
      let* (exit_code, _output) = Container.exec container ["test"; "-f"; "/tmp/" ^ filename] in
...
let suite =
  [
    Alcotest_lwt.test_case "copy content to container" `Slow test_copy_content_to_container;
    Alcotest_lwt.test_case "copy file to container" `Slow test_copy_file_to_container;
  ]
