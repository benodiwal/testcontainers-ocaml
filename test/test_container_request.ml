...
let test_create _switch () =
  ...
  Alcotest.(check string)
    "image" "nginx:alpine"
    (Container_request.image request);
  Alcotest.(check int)
    "no exposed ports" 0
    (List.length (Container_request.exposed_ports request));
  Alcotest.(check int)
    "no env vars" 0
    (List.length (Container_request.environment request));
  Lwt.return_unit
...
let test_with_env _switch () =
  ...
  Alcotest.(check bool)
    "password set" true
    (List.mem ("POSTGRES_PASSWORD", "secret") env);
  Alcotest.(check bool)
    "user set" true
    (List.mem ("POSTGRES_USER", "admin") env);
  Lwt.return_unit
...
let test_with_cmd _switch () =
  ...
  Alcotest.(check bool)
    "cmd set" true
    (Option.is_some (Container_request.command request));
  Alcotest.(check (list string))
    "cmd value" [ "echo"; "hello" ]
    (Option.get (Container_request.command request));
  Lwt.return_unit
...
let test_with_entrypoint _switch () =
  ...
  Alcotest.(check bool)
    "entrypoint set" true
    (Option.is_some (Container_request.entrypoint request));
  Lwt.return_unit
...
let test_with_working_dir _switch () =
  ...
  Alcotest.(check (option string))
    "working dir" (Some "/app")
    (Container_request.working_dir request);
  Lwt.return_unit
...
let test_with_user _switch () =
  ...
  Alcotest.(check (option string))
    "user" (Some "nobody")
    (Container_request.user request);
  Lwt.return_unit
...
let test_with_name _switch () =
  ...
  Alcotest.(check (option string))
    "name" (Some "my-nginx")
    (Container_request.name request);
  Lwt.return_unit
...
let test_with_mount _switch () =
  ...
  Alcotest.(check int)
    "one mount" 1
    (List.length (Container_request.mounts request));
  Lwt.return_unit
...
let test_with_startup_timeout _switch () =
  ...
  Alcotest.(check (float 0.1))
    "timeout" 120.0
    (Container_request.startup_timeout request);
  Lwt.return_unit
...
let test_with_auto_remove _switch () =
  ...
  Alcotest.(check bool)
    "auto_remove" true
    (Container_request.auto_remove request);
  Lwt.return_unit
...
let suite =
  [
    ...
    Alcotest_lwt.test_case "with_startup_timeout" `Quick
      test_with_startup_timeout;
